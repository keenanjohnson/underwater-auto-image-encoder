...
# Based on: https://github.com/LintaoPeng/U-shape_Transformer_for_Underwater_Image_Enhancement
# Paper: U-shape Transformer for Underwater Image Enhancement
...
class IntermediateSequential(nn.Sequential):
    """Sequential that can optionally return intermediate outputs"""
...
class Flatten(nn.Module):
    """Flatten layer for attention"""
    def forward(self, x):
        return x.view(x.size(0), -1)


# ==============================================================================
# Equalized Convolution Layers
# ==============================================================================
...
class _equalized_conv2d(nn.Module):
    ...
    def forward(self, x):
        return F.conv2d(input=x, weight=self.weight * self.scale,
...
class conv_block(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, use_eql=True):
        ...
        if use_eql:
            ...
            self.conv_1 = nn.Conv2d(in_ch, out_ch, (3, 3), padding=1, bias=True)
            self.conv_2 = nn.Conv2d(out_ch, out_ch, (3, 3), padding=1, bias=True)
            self.conv_3 = nn.Conv2d(out_ch, out_ch, (3, 3), padding=1, bias=True)
...
# ==============================================================================
# SGFMT - Spatial-wise Global Feature Modeling Transformer
# ==============================================================================
...
class PreNormDrop(nn.Module):
    """Pre-normalization with dropout wrapper"""
...
# ==============================================================================
# CMSFFT - Channel-wise Multi-Scale Feature Fusion Transformer
# ==============================================================================

class Channel_Embeddings(nn.Module):
    """Channel embeddings for CMSFFT"""
...
class Reconstruct(nn.Module):
    """Reconstruct features from patches"""
...
class Attention_org(nn.Module):
    """Multi-scale attention for CMSFFT"""
...
class Block_ViT(nn.Module):
    """Vision Transformer block for CMSFFT"""
...
class ChannelTransformer(nn.Module):
    """Channel-wise Multi-Scale Feature Fusion Transformer"""
...
# ==============================================================================
# Main U-shape Transformer Generator
# ==============================================================================
...
class UShapeTransformer(nn.Module):
    ...
    This model combines a U-Net style encoder-decoder with two transformer modules:
    - CMSFFT: Channel-wise Multi-Scale Feature Fusion Transformer
    - SGFMT: Spatial-wise Global Feature Modeling Transformer

    Note: The original model returns multi-scale outputs for GAN training.
    For non-GAN training, use return_single=True to get only the final output.
    """
    ...
    def __init__(self,
        ...
        # Spatial-wise Global Feature Modeling Transformer (SGFMT)
        ...
        # ModuleList for multi-scale feature processing
    ...
    def forward(self, x):
        ...
        # Channel-wise transformer-based attention (CMSFFT)
        e1, e2, e3, e4, att_weights = self.mtc(e1, e2, e3, e4)

        # Spatial-wise transformer-based attention (SGFMT)
        residual = e5
        e5 = self.bn(e5)
        e5 = self.relu(e5)
        e5 = self.Conv_x(e5)
        ...
        if self.return_single:
            # Return only the final (highest resolution) output with sigmoid
            return torch.sigmoid(output[-1])
        else:
            # Return all multi-scale outputs (for GAN training)
            return output


# Alias for compatibility
Generator = UShapeTransformer
